					...
					return (
						<div>
							<h4>
								...
								<span style={{ color: 'green', marginLeft: '3%' }}>Correct</span>
							</h4>
						</div>
					);
				})}
				...
				{wrongAnswer.map((val) => {
					return (
						<div>
							<h4>
								{val[0]} <span style={{ marginLeft: '2%' }}>{val[1]}</span>
								<span style={{ color: 'red', marginLeft: '3%' }}>Incorrect</span>
							</h4>
						</div>
					);
				})}
...
export default function Main() {
	...
	const submit = () => {
		...
		if (selectedValue === filter[0].item) {
			...
			if (find) {
				wrongAnswer = wrongAnswer.filter((val) => val[0] !== value[count].questions);
			}

			const same = correctAnswer.findIndex((val) => val[0] === value[count].questions);
			if (same > -1) {
			...
			if (find) {
				correctAnswer = correctAnswer.filter((val) => val[0] !== value[count].questions);
			}

			const same = wrongAnswer.findIndex((val) => val[0] === value[count].questions);
			if (same > -1) {
